...
class GripperController:
    ...
    def _write_regs(self, addr, count, val_tab, val_type=None):
        if self.id <= 0: return
...
class TeleopController:
    def __init__(self):
        ...
        # --- 控制参数配置 ---
        self.LOOP_RATE = 30.0       # 控制频率 (Hz), Dobot Servo 建议 30-50Hz
        self.MOUSE_SENSITIVITY = 0.1 # 鼠标移动 -> mm 的比例
        self.SCROLL_SENSITIVITY = 2.0 # 滚轮 -> 角度的比例
        self.KEY_XYZ_STEP = 1.0     # 键盘控制 Z (Space/Ctrl) 的步长 (mm)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if 'a' in self.keys_pressed:
                self.target_pose[3] += self.KEY_ROT_STEP
            if 'd' in self.keys_pressed:
                self.target_pose[3] -= self.KEY_ROT_STEP
            
            # A/D -> Ry (Roll)
            if 'w' in self.keys_pressed:
                self.target_pose[4] -= self.KEY_ROT_STEP
